...
class ShopProduct(Entity, SoftDeleteMixin):
    ...
    specifications: Any = field(default=None, metadata=dict(description='商品规格(JSON格式，包含规格名称和规格值)'))
    services: Any = field(default=None, metadata=dict(description='售后服务(JSON格式，包含服务类型和适用状态)'))
    attributes: Any = field(default=None, metadata=dict(description='扩展属性(JSON格式，包含属性名和属性值)'))
